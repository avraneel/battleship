...
describe("Ship validity", () => {
  test("checks for invalid ship", () => {
    expect(gb.isValidShip(3, 5, 7, false)).toBe(false);
  });

  test("checks for valid ship", () => {
    expect(gb.isValidShip(3, 5, 3, false)).toBe(true);
  });
});
...
describe("Ship Placement", () => {
  ...
  test("place a ship whose non-starting position is occupied", () => {
    expect(() => gb.placeShip(2, 6, 2, true)).toThrow();
    expect(gb.board.slice(2, 5).map((x) => x[6])).toStrictEqual([0, 1, 0]);
  });
